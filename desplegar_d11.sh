...
USER_WEB=www-data
D11_REPO=https://github.com/sgargallo/integra_d11base.git
...
if [ $# -eq 0 ]; then
	>&2 echo "Nombre del sitio web no especificado. Ejecute ./desplegar_d11.sh nombreSitioWeb"
	exit 1
fi
...
sudo -u $USER_WEB git clone $D11_REPO $1

#Accedemos a la carpeta del sitio web
cd $RUTA_WEB/$1/

#Ejecutamos instalación de Drupal con composer
echo "Desplegando Drupal 11 utilizando composer..."
